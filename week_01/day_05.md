...
graph TD
    ...
    subgraph 错误处理
        error1[词法错误] --> errHandler1[标记错误位置]
        error2[语法错误] --> errHandler2[提示语法规则]
        error3[语义错误] --> errHandler3[类型不匹配提示]
    end

    %% 样式定义
    style A fill:#4A90E2,color:white,stroke-width:0
    style I fill:#4A90E2,color:white,stroke-width:0
    style C fill:#7EC8E3,stroke:#4A89BF
    style D fill:#7EC8E3,stroke:#4A89BF
    style E fill:#7EC8E3,stroke:#4A89BF
    style F fill:#88D498,stroke:#4CAF50
    style G fill:#FFB74D,stroke:#FF9800
    style H fill:#BA68C8,stroke:#9C27B0

    classDef process fill:#7EC8E3,stroke:#4A89BF;
    classDef middle fill:#88D498,stroke:#4CAF50;
    classDef optimize fill:#FFB74D,stroke:#FF9800;
    classDef final fill:#BA68C8,stroke:#9C27B0;
    classDef error fill:#FF5252,stroke:#D32F2F;

    linkStyle 0,1,2,3,4,5,6 stroke:#4A89BF,stroke-width:2px;
```
