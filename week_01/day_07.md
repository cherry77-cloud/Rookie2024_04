...
#### 核心优势
- `epoll` 在 `Linux` 高并发场景下性能卓越，支持 `ET` 模式和 `EPOLLONESHOT`。
- 适用场景：`Web` 服务器（如 `Nginx`）、实时通信系统等。
- `ET` 模式需搭配非阻塞 `I/O`。
- 正确处理 `EAGAIN/EWOULDBLOCK` 错误。
- 避免在 `ET` 模式下遗漏未处理的数据。
- 通过合理使用 `epoll`，可显著提升程序的并发处理能力和资源利用率。
---

| **特性**               | **`select`**                              | **`poll`**                                | **`epoll`**                              |
|------------------------|-----------------------------------------|-----------------------------------------|----------------------------------------|
| **事件管理方式**       | 位掩码（`fd_set`）                     | 结构体数组（`pollfd`）                 | 内核红黑树 + 就绪队列                |
| **最大文件描述符数**   | `1024`（可调整但不稳定）                 | `65535`（系统资源限制）                  | `65535`（系统资源限制）                 |
| **时间复杂度**         | `O(n)`                                   | `O(n)`                                   | `O(1)`（就绪事件获取）                  |
| **触发模式**           | `LT`（电平触发）                         | `LT`（电平触发）                         | `LT/ET`（边沿触发）                     |
